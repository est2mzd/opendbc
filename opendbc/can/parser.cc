...
/*
et_raw_value 関数は、CANメッセージ（msg）から任意のビット位置に存在する 信号の
生値（raw value） を抽出するための関数です。
主に Signal 構造体で定義されたビット範囲とエンディアン情報を元に値を復元
*/
  ...
  */
  while (i >= 0 && i < msg.size() && bits > 0) {
    ...
    */
    ...
    // 取り出した d を、**まだ空いている ret の中の上位側（残りビット数の位置）**に左シフトして配置する。
    ret |= d << (bits - size);
